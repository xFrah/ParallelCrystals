...
void reset_linked_lists(ListHead*** grid) {
...
void appendNode(ListHead* listHead, Particle* newHead) {
    Particle* oldHead;
    do {
...
void makeLinkedLists(ListHead*** grid, Particle* particles) {
...
void sort_single_cell_insertionSort(ListHead*** grid) {
    int row = blockIdx.y * blockDim.y + threadIdx.y;
    int col = blockIdx.x * blockDim.x + threadIdx.x;
...
void check_for_collisions(ListHead*** grid) {
    int row = blockIdx.y * blockDim.y + threadIdx.y;
    int col = blockIdx.x * blockDim.x + threadIdx.x;
...
void print_linked_lists(ListHead*** grid) {
    int counter = 0;
...
void move_particles_kernel(Particle* particles, curandState* states) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int num_threads = blockDim.x * gridDim.x;
    int start = (idx * d_config.NUM_PARTICLES) / num_threads;
    int end = ((idx + 1) * d_config.NUM_PARTICLES) / num_threads;
...
void init_particles_kernel(Particle* particles, curandState* states) {
...
void initializeGrid(ListHead*** grid) {
    int row = blockIdx.y * blockDim.y + threadIdx.y;
    int col = blockIdx.x * blockDim.x + threadIdx.x;
