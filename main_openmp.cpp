...
void append_node(ListHead *listHead, Particle *newHead) {
    Particle *oldHead;
    do {
        oldHead = listHead->head;
        newHead->next_particle = oldHead;
    } while (InterlockedCompareExchange((unsigned long long int *)&(listHead->head), (unsigned long long int)newHead, (unsigned long long int)oldHead) != (unsigned long long int)oldHead);
}
